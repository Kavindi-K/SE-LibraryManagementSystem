...
const Homepage = () => {
  ...
  const handleBecomeMemberClick = () => {
    if (isExistingMember) {
      // Redirect existing members to their profile
  ...
  const handleMemberRegistration = async (e) => {
    ...
    try {
      ...
      if (memberResponse.data.success) {
        setSuccess('Member registration successful! Redirecting to your member portal...');
        setMemberFormData({
          email: user.email || '',
          password: '',
          membershipType: 'BASIC'
      ...
      if (error.response?.status === 400 && error.response?.data?.message?.includes('already a member')) {
        // User is already a member, redirect to profile
        setSuccess('Redirecting to your member profile...');
        setTimeout(() => {
          setShowMemberModal(false);
          navigate('/member-profile');
        }, 1500);
  ...
  const handleMemberIdLogin = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8081/api/members/login', {
        memberId: memberIdLoginData.memberId
      });
      ...
      if (response.data.success) {
        ...
        // Update user data with member info
